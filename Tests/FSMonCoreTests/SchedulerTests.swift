...
final class SchedulerTests: XCTestCase {
    final class Fake: Sampler {
        ...
        let action: () throws -> SamplerOutcome
        private let lock = NSLock()
        private var runs = 0
        var count: Int { lock.lock(); defer { lock.unlock() }; return runs }
        init(_ name: String, interval: TimeInterval, action: @escaping () throws -> SamplerOutcome = { .ok }) {
    ...
    func testIndependentIntervalsAndStop() throws {
        ...
        let fast = Fake("fast", interval: 0.02) { fastDone.fulfill(); return .ok }
        let slow = Fake("slow", interval: 0.12) { slowDone.fulfill(); return .degraded("test detail") }
    ...
    func testBusySamplerSkipsTicksAndDoesNotBlockOtherSampler() throws {
        ...
        let slow = Fake("blocked", interval: 0.01) { entered.fulfill(); _ = gate.wait(timeout: .now() + 3); return .ok }
        let fast = Fake("fast", interval: 0.02) { fastRuns.fulfill(); return .ok }
    ...
    func testThrownErrorIsRecorded() throws {
        ...
        let fake = Fake("error", interval: 0.01) { ran.fulfill(); throw Failure() }
